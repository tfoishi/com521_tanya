...
##is avg of x different than avg of y 
##when x goes up, then y goes up.
## null: the mean of x = mean of y
## are these two groups drawn from the same population (no)


#PC2. Estimate how correlated x and y are with each other.

cor(tanya.week3$x, tanya.week3$y, method= "spearman")
#spearman is always safe. 
...
yxlm1 <- lm(tmp.week3$y ~ tmp.week3$x, data = tmp.week3) #linear model with x predicting y 
yxlm2.i <- lm(tmp.week3$y ~ tmp.week3$x + tmp.week3$i, data = tmp.week3) #i as a control variable 
yxlm3.ij <- lm(tmp.week3$y ~ tmp.week3$x + tmp.week3$i + tmp.week3$j, data = tmp.week3) #i and j and control variables 
yxlm4.ijk <- lm(tmp.week3$y ~ tmp.week3$x + tmp.week3$i + tmp.week3$j + tmp.week3$k.factor , data = tmp.week3) #i, j, k as control variables 

summary.lm(yxlm1)
#1 in crease in x is a 4.3 icrease in  y #
...
plot(tmp.week3$k.factor, residuals(yxlm4.ijk))
